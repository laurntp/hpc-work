...
print(sys.path)
from utils.dataLoader import LandmarksDataset_NoNormExpand, ToTensorCL, ToTensorLH, Rescale, RandomScale, AugColor, Rotate
...
def main(config):
    ...
    img_path = os.path.join(config.train_csv_path, 'Images')
    label_path = os.path.join(config.train_csv_path, 'landmarks')

    train_dataset = LandmarksDataset_NoNormExpand(img_path=img_path,
                                     label_path=label_path,
                                     transform = transforms.Compose([Rescale(config.image_width),
                                                 ToTensorCL()])
                                     )

    img_path = os.path.join(config.valid_csv_path, 'Images')
    label_path = os.path.join(config.valid_csv_path, 'landmarks')
    val_dataset = LandmarksDataset_NoNormExpand(img_path=img_path,
                                     label_path=label_path,
                                     transform = transforms.Compose([
                                                 Rescale(config.image_width),
                                                 ToTensorCL()])
                                     )
    ...
    valid_loader = torch.utils.data.DataLoader(val_dataset, batch_size = config.batch_size, num_workers = config.num_workers)
    ...
    writer = SummaryWriter(log_dir=config.save_model_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_csv_path', type=str, default="/rds/user/hpcpin1/hpc-work/SegProject/Datasets/Cytomine/Cephalo/Train")
    parser.add_argument('--valid_csv_path', type=str, default="/rds/user/hpcpin1/hpc-work/SegProject/Datasets/Cytomine/Cephalo/Val")
